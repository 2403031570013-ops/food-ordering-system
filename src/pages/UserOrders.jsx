...
import { ArrowLeft, Clock, ShoppingBag, MapPin, FileText, CheckCircle, XCircle, Store, RefreshCw, ChevronRight } from 'lucide-react';
                        ...
                        <AnimatePresence>
                            {orders.map((order, index) => (
                                ...
                                >
                                    ...
                                    {/* Status Timeline (if order is active) */}
                                    {['pending', 'accepted', 'preparing', 'ready', 'out-for-delivery'].includes(order.status) && (
                                            ...
                                            <div className="md:w-72 md:border-l border-slate-100 md:pl-8 space-y-6">
                                                ...
                                                <div className="pt-2">
                                                    {order.invoiceUrl ? (
                                                        ...
                                                        <div className="text-xs text-slate-400 italic text-center py-2 bg-slate-50/50 rounded-lg border border-dashed border-slate-200">
...
function OrderStatusBadge({ status }) {
    ...
    return (
        <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide border ring-2 ring-opacity-50 shadow-sm ${styles[status] || styles.pending}`}>
            {labels[status] || status.replace(/-/g, ' ')}
        </span>
    );
}

// Order Timeline Component for Active Orders
        ...
        <div className="relative">
            ...
            <div className="relative flex justify-between">
                {steps.map((step, index) => {
                    ...
                    const label = step === 'pending' ? 'Placed' :
                        step === 'accepted' ? 'Confirmed' :
                            step === 'preparing' ? 'Cooking' :
                                step === 'ready' ? 'Ready' :
                                    step === 'out-for-delivery' ? 'On Way' : 'Delivered';

                    const Icon = step === 'pending' ? FileText :
                        step === 'accepted' ? CheckCircle :
                            step === 'preparing' ? Clock :
                                step === 'ready' ? CheckCircle :
                                    step === 'out-for-delivery' ? MapPin : ShoppingBag;
